...
class ImageHelper:
    ...
    def get_rmac_features(self, I, R, net):
        ...
        # feature map
        # h = Variable(np.array(net.blobs['res5c'].data)
        h = net(Variable(torch.from_numpy(I).cuda()))
        h = h.cpu().data
        # h =  np.squeeze(net.blobs['res5c'].data)
...
class Dataset:
    ...
    def load(self):
        ...
        for e in self.query_imagenames:
            ...
            good = set([e for e in self.index_imagenames if house_num == e.split('_')[0]])
            self.relevants[q_name] = [i for i in range(len(self.index_imagenames)) if
                                      self.index_imagenames[i] in good]
            # self.non_relevants[q_name] = [i for i in range(len(self.index_imagenames)) if
            #                               self.index_imagenames[i] not in good]
    ...
    def score(self, sim, temp_dir):
        ...
        ls_top1 = []
        ls_top10 = []
        for i in range(len(self.query_imagenames)):
            ap_top1, ap_top10 = self.score_rnk_partial(i, idx[i], temp_dir)
            ls_top1.append(ap_top1)
            ls_top10.append(ap_top10)

        for i in range(len(self.query_imagenames)):
            print("{0}: {1:.1f}, {2:.2f}".format(self.query_imagenames[i], ls_top1[i], ls_top10[i]))
        print(20 * "-")
        print("Mean: {0:.6f}, {1:.1f}".format(100 * np.sum(ls_top1)/len(ls_top1), 100 * np.sum(ls_top10)/len(ls_top10)))
        return 100 * np.sum(ls_top1)/len(ls_top1)
    ...
    def score_rnk_partial(self, i, idx, temp_dir):
        ...
        ap_top1 = self.compute_ap_topn(rnk, i, n=1)
        ap_top10 = self.compute_ap_topn(rnk, i, n=10)
        return ap_top1, ap_top10

    def compute_ap_topn(self, rank_list, i, n):
        if n == 1:
            query_name = self.q_names[i].split('_')[0]
            rank_name = rank_list[0].split('_')[0]
            if query_name.isdigit() and rank_name.isdigit():
                ap_topn = 1.0
            else:
                query = ''.join(re.findall(r'[A-Za-z]', query_name))
                rank = ''.join(re.findall(r'[A-Za-z]', rank_name))
                if rank == query:
                    ap_topn = 1.0
                else:
                    ap_topn = 0.0
            ...
            #     query_name = self.q_names[i].split('_')[0]
            #     rank_name = rank_list[j].split('_')[1]
            #     if query_name.isdigit() and rank_name.isdigit():
            #         intersect_size += 1
            #         recall = intersect_size / len(self.relevants[self.q_names[i]])
            #         precision = intersect_size / (i + 1)
            #         ap_topn += (recall - old_recall) * ((old_precision + precision) / 2)
            #         old_recall = recall
...
if __name__ == '__main__':
    ...
    # street view
    index_path = '/ocean/malan/image_retrieve/datasets/house/hx_data'
    query_path = '/ocean/malan/image_retrieve/datasets/house/test'
    ...
    # Score
    dataset.score(sim, args.temp_dir)
